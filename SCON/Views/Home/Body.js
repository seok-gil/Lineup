...
function PlayerCard({ card, user_code, index, navigation }) {
	return (
		<TouchableOpacity onPress={() => navigation.navigate('Player')} style={{ flexDirection: 'row' }}>
...
function EmptyCard({ user_code, index, navigation }) {
	return (
		<TouchableOpacity onPress={() => navigation.navigate('SearchStack')}  style={{ flexDirection: 'row' }}>
			{index == 0 && <Text style={{ flex: 1 }}>@@@null@@@</Text>}
			{index != 0 && <Text style={{ flex: 1 }}>@@@prev@@@</Text>}
...
function FollowCardList({ Data, navigation }) {
	...
	if (user_code == 1)
		view.push(<PlayerMyCard navigation={navigation}/>)
	const playercard = () => {
		for (let i = 0; i < 3; i++) {
			if (follow[i])
				view.push(<PlayerCard user_code={user_code} card={follow[i]} index={i + user_code} navigation={navigation} />)
			else
				view.push(<EmptyCard user_code={user_code} index={i + user_code} navigation={navigation} />)
		}
		return (view)
	}
...
export function Body({ Data, navigation }) {
	...
	return (
		<View style={{ flex: 3 }}>
			{user_code == 1 && goPlayer == true && (<GoPlayer navigation={navigation} setgoPlayer={setgoPlayer} />)}
